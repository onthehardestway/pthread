...
int main (int argc, char *argv[])
{
  ...
  /*
   * If an argument is specified, interpret it as the number of
   * seconds for wait_thread to sleep before signaling the condition
   * variable. You can play with this to see the condition wait below
   * time out or wake normally.
   */
  if (argc > 1)
    {
  ...
  /*
   * Wait on the condition variable for 2 seconds, or until signaled
   * by the wait_thread. Normally, wait_thread should signal. If you
   * raise "hibernation" above 2 seconds, it will time out.
   */
