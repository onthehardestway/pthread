...
void alarm_insert(alarm_t *alarm) {
   ...
   * there. ("next" is NULL, and "last" points to the link field of
   * the last item, or to the list header.)
   */
  if (next == NULL) {
    *last = alarm;
    alarm->link = NULL;
  ...
  for (next = alarm_list; next != NULL; next = next->link) {
    printf("%d(%ld)[\"%s\"] ", next->seconds, next->time - time(NULL),
           next->message);
  }
  printf("]\n");
#endif  // DEBUG
...
void *alarm_thread(void *arg) {
   ...
   * disintegrated when the process exits. Lock the mutex at the start
   * -- it will be unlocked during condition waits, so the main thread
   * can insert alarms.
   */
