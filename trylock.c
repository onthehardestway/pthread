...
/*
 * Thread start routine that repeatedly locks a mutex and increments a
 * counter.
 */
void *counter_thread (void *arg)
{
    ...
    /*
     * Until end_time, increment the counter each seconds. Instead of
     * just incrmenting the counter, it sleeps for another second with
     * the mutex locked, to give monitor_thread a reasonable chance of
     * running.
     */
...
/*
 * Thread start routine to "moniter" the counter. Every 3 seconds, try
 * to lock the mutex and read the counter. If the trylock fails, skip
 * this cycle.
 */
...
int main (int argc, char *argv[])
{
    ...
    if (status != 0) {
	err_abort (status, "Join mointor thread");
    }
    return 0;
}
